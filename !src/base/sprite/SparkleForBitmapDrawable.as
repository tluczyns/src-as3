package base.sprite {
	...
	public class SparkleForBitmapDrawable extends Sprite {
		...
		public function anim(isRepeat: Boolean, repeatDelay: Number = 0, time: Number = 1, startDelay: Number = 0): void {
			if (!isRepeat) this.shpGradientSparkle.x = - this.shpGradientSparkle.width;
			TweenLite.to(this.shpGradientSparkle, time, {x: this.bmpMaskForSparkle.width, ease: Linear.easeNone, repeat: [0, -1][uint(isRepeat)], repeatDelay: repeatDelay, delay: startDelay});
		}
		
		public function destroy(): void {
			TweenLite.killTweensOf(this.shpGradientSparkle);
