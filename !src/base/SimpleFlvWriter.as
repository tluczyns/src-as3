	...
	public class SimpleFlvWriter
	{
		...
		public function createFile(pWidth:int, pHeight:int, pFramesPerSecond:Number, pDurationInSeconds:Number=0):void
		{
			...
			*/
			...
			this.baFlv = new ByteArray();
			this.baFlv.writeBytes(header());
			this.baFlv.writeUnsignedInt(previousTagSize)
			this.baFlv.writeBytes( flvTagOnMetaData() );
			/*
		...
		public function saveFrame(pBitmapData:BitmapData):void
		{
			...
			this.baFlv.writeUnsignedInt( previousTagSize );
			this.baFlv.writeBytes( flvTagVideo() );	
			/*
			fs.writeUnsignedInt( previousTagSize );
