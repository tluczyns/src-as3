package baseStarling.gallery {
	...
	import baseStarling.btn.BtnHit;
	import base.types.DictionaryExt;
	import starling.events.Event;
	//import starling.display.Shape;
	//swf address gateway
	import baseStarling.vspm.StateModel;
	import baseStarling.vspm.EventStateModel;
	...
	public class MultiCircleGallery extends Sprite {
		...
		protected function createItems(): void {
			/*this.btnCover = this.createBtnCover();
			this.addChild(this.btnCover);*/
		...
		/*private function createBtnCover(): BtnHit {
			var shpBtnCover: Shape = new Shape();
			shpBtnCover.graphics.beginFill(0x000000, 0.5);
			shpBtnCover.graphics.drawRect(0, 0, this.stage.stageWidth, this.stage.stageHeight);
			shpBtnCover.graphics.endFill();
			var btnCover: BtnHit = new BtnHit(shpBtnCover);
			return btnCover;
		}*/
		...
		private function removeItems(): void {
			...
			/*this.removeChild(this.btnCover);
		...
		private function addEventsToVecArrows(): void { 
			for (var i: uint = 0; i < this.vecArrows.length; i++) {
				var arrows: ArrowsGalleryMulti = this.vecArrows[i];
				arrows.addEventListener(ArrowsGalleryMulti.ON_ARROWS_RELEASED, this.onArrowsReleased);
			}
		}
		
		private function onArrowsReleased(e: Event):void {
		...
		private function removeEventsFromVecArrows(): void { 
			for (var i: uint = 0; i < this.vecArrows.length; i++) {
				var arrows: ArrowsGalleryMulti = this.vecArrows[i];
				arrows.removeEventListener(ArrowsGalleryMulti.ON_ARROWS_RELEASED, this.onArrowsReleased);
			}
		}
		...
		private function onGestureSwipe(e: TransformGestureEvent): void {
			if (e.offsetY == 0) {
				var targetNumItem: int = this.num - MathExt.getSign(e.offsetX);
				if (targetNumItem < 0) targetNumItem = GalleryMain.COUNT_ITEMS - 1;
				else if (targetNumItem > GalleryMain.COUNT_ITEMS - 1) targetNumItem = 0;
				this.removeAutomaticChange();
				this.dispatchEvent(new EventGallery(EventGallery.SELECTED_ITEM_CHANGED, targetNumItem,  true));
			}
		}
