package baseStarling.tf {
	...
	public class TextFieldUtils extends Singleton {
		...
		static public function createTextField(objTextField: Object = null): TextField {
			...
			objTextField.bold = Boolean(objTextField.bold);
			objTextField.hAlign = objTextField.hAlign || HAlign.LEFT;
			objTextField.vAlign = objTextField.vAlign || VAlign.TOP;
			objTextField.multiline = Boolean(objTextField.multiline);
			objTextField.leading = objTextField.leading || 0;
			var tf: TextField = new TextField(objTextField.width, objTextField.height, objTextField.text, objTextField.fontName, objTextField.fontSize, objTextField.color, objTextField.bold);
			tf.leading = objTextField.leading;
			tf.hAlign = objTextField.hAlign;
            tf.vAlign = objTextField.vAlign;
		...
		static public function fitTextToMaxWidthHeight(tf: TextField, isWidthHeight: uint, text: String, startMaxFontSize: uint, maxWidthHeightTextWhenStartDecreaseFontSize: Number, minFontSize: uint = 4, isSetHtmlText: Boolean = false): Number {
			if (text != null) {
				...
				tf.text = text;
				tf.fontSize = startMaxFontSize;
				while ((tf.textBounds[["width", "height"][isWidthHeight]] > maxWidthHeightTextWhenStartDecreaseFontSize) && (tf.fontSize > minFontSize)) {
					tf.fontSize -= 1;
				}
