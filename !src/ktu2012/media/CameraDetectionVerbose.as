...
package ktu.media {
	 ...
	 *
	 */
	public class CameraDetectionVerbose extends EventDispatcher {
		...
		private function callOutput(strOutput: String ): void {
			if (output != null) output(strOutput);
		}
		...
		public function begin ():void {
			callOutput("CameraDetection::begin()");
			_video = new Video ();
			_numCameras = Camera.names.length;
			callOutput("\tfound " + _numCameras + " cameras");
...
/* FAIL */		dispatch (CameraDetectionResult.NO_CAMERAS);		// just in case?
			...
			callOutput("\tCamera.muted = " + _camera.muted);
			if (!_camera.muted) {
		...
		public function dispose ():void {
			callOutput("CameraDetection::dispose()");
			if (_video) {
		...
		private function getDefaultCamera ():void {
			...
			if (_camera) _defaultCameraName = _camera.name;
		}
		/** @private
		 *
		...
		private function checkCamera ():void {
			callOutput("CameraDetection::checkCamera()");
			callOutput("\t" + _camera.name);
			...
			callOutput("activity:average\tfps:average");
		}
		/** @private
		 *
		...
		private function tickCheckCamera (e:TimerEvent):void {
			switch (e.type) {
				case TimerEvent.TIMER:
					...
					callOutput("act: " + _camera.activityLevel + ":" + _camActivityAverage + "\tfps: " + _camera.fps + ":" + _camFPSAverage);
...
/* SUCCESS */				dispatch (CameraDetectionResult.SUCCESS);
		...
		private function askPermission():void {
			callOutput("CameraDetection::askPermission()");
		...
		private function havePermissions():void {
			callOutput("CameraDetection::permissionGranted()");
			constructTimer();
			checkCamera ();
		}
		/** @private
		...
		private function tickPermissionsCheck (e:TimerEvent):void {
			if (!_camera.muted) { // permision was granted
				...
				if ( _timer.currentCount >= _permissionsDenyCount ) {
					callOutput("CameraDetection::permissionDenied()");
...
/* FAIL */			dispatch (CameraDetectionResult.NO_PERMISSION);
		...
		private function onCamActivity(e:ActivityEvent):void {
			if (e.activating) dispatch (CameraDetectionResult.SUCCESS);//TŁ
		}
		/** @private
		 *
