package com.senocular.display {
	...
	public class TransformTool extends Sprite {
		...
		private var isForTextField: Boolean;
		 ...
		 */
		public function TransformTool(isControlsOnlyAtCorners: Boolean = false, isForTextField: Boolean = false) {
			this.isForTextField = isForTextField;
			createControls(isControlsOnlyAtCorners);
		}
		...
		// Setup
		private function createControls(isControlsOnlyAtCorners: Boolean):void {
			...
			if (!this.isForTextField) addToolControl(cursors, _rotationCursor, false);
			addToolControl(cursors, _scaleCursor, false);
			if (!this.isForTextField) addToolControl(cursors, _skewCursor, false);
		...
		public function scaleManually(scaleX: Number, scaleY: Number): void {
			var distortH:Point = distortOffset(new Point(- scaleX * this.target.width / 2, 0), - this.target.width / 2);
			var distortV:Point = distortOffset(new Point(0, - scaleY * this.target.height / 2), - this.target.height / 2);
			_toolMatrix.a = distortH.x;
			_toolMatrix.b = distortH.y;
			_toolMatrix.c = distortV.x;
			_toolMatrix.d = distortV.y;
		...
		public function skewManually(skewX: Number, skewY: Number): void {
			_toolMatrix.b += (skewY * Math.PI / 180);
			_toolMatrix.c += (skewX * Math.PI / 180);
			completeInteraction(true);
			dispatchEvent(new Event(TRANSFORM_TOOL));
		}
		...
		public function apply():void {
			if (_target) {
				...
				// set target's matrix
				if (!this.isForTextField) _target.transform.matrix = applyMatrix;
				else {
					_target.x = applyMatrix.tx;
					_target.y = applyMatrix.ty;
					_target.rotation = MatrixTransformer.getRotation(applyMatrix);
					_target.scaleX = MatrixTransformer.getScaleX(applyMatrix);
					_target.scaleY = MatrixTransformer.getScaleY(applyMatrix);
				}
